...
enum fundamentals {
  ...
  unit_shift = 2,
  tuple_flags_bits = 2,
  max_field_bytes = unit_size * (UINT16_MAX - 1),
  max_tuple_bytes = unit_size * (UINT16_MAX + 1),
  max_fields = (1 << (16 - tuple_flags_bits)) - 1,
  buffer_enough =
      sizeof(size_t) * 16 + max_tuple_bytes + max_fields * unit_size,
  buffer_limit = max_tuple_bytes * 2,
};
