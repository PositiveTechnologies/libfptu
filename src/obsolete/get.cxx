...
fptu_datetime_t fptu_field_datetime(const fptu_field *pf) {
  if (unlikely(fptu_field_type(pf) != fptu_datetime))
    return FPTU_DENIL_TIME;

  return fptu_datetime_t(pf->payload()->u64);
}
...
fptu_datetime_t fptu_get_datetime(fptu_ro ro, unsigned column, int *error) {
