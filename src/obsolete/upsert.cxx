...
static __hot fptu_field *fptu_find_dead(fptu_rw *pt, std::size_t units) {
...
static __hot fptu_field *fptu_append(fptu_rw *pt, uint_fast16_t ct,
  ...
  if (likely(units)) {
    std::size_t offset = (size_t)(&pt->units[pt->tail].data - pf->body);
...
static __hot fptu_field *fptu_emplace(fptu_rw *pt, uint_fast16_t ct,
  ...
  if (pf) {
    std::size_t avail = fptu_field_units(pf);
...
static __hot fptu_takeover_result fptu_takeover(fptu_rw *pt, uint_fast16_t ct,
  ...
  std::size_t avail = fptu_field_units(result.pf);
  if (likely(avail == units)) {
...
static __inline void fptu_cstrcpy(fptu_field *pf, std::size_t units,
...
static fptu_error fptu_upsert_32(fptu_rw *pt, fptu_tag_t tag,
                                 uint_fast32_t value) {
  assert(tag_match_fixedsize(tag, 1));
  assert(!fptu_tag_is_deleted(tag));
...
static fptu_error fptu_upsert_64(fptu_rw *pt, fptu_tag_t tag,
                                 uint_fast64_t value) {
...
fptu_error fptu_upsert_datetime(fptu_rw *pt, unsigned col,
                                fptu_datetime_t value) {
  if (unlikely(col > fptu_max_cols))
    return FPTU_EINVAL;
...
fptu_error fptu_upsert_string(fptu_rw *pt, unsigned col, const char *text,
  ...
  std::size_t units = bytes2units(length + 1);
...
fptu_error fptu_upsert_opaque(fptu_rw *pt, unsigned col, const void *value,
  ...
  std::size_t units = bytes2units(bytes) + 1;
...
fptu_error fptu_upsert_nested(fptu_rw *pt, unsigned col, fptu_ro ro) {
  ...
  std::size_t units = (size_t)ro.units[0].varlen.brutto + 1;
...
static fptu_error fptu_update_32(fptu_rw *pt, fptu_tag_t tag,
                                 uint_fast32_t value) {
  assert(tag_match_fixedsize(tag, 1));
  assert(!fptu_tag_is_deleted(tag));
...
static fptu_error fptu_update_64(fptu_rw *pt, fptu_tag_t tag,
                                 uint_fast64_t value) {
  assert(tag_match_fixedsize(tag, 2));
  assert(!fptu_tag_is_deleted(tag));
...
fptu_error fptu_update_datetime(fptu_rw *pt, unsigned col,
                                fptu_datetime_t value) {
  if (unlikely(col > fptu_max_cols))
    return FPTU_EINVAL;
...
static fptu_error fptu_insert_32(fptu_rw *pt, fptu_tag_t tag,
                                 uint_fast32_t v) {
  assert(tag_match_fixedsize(tag, 1));
  assert(!fptu_tag_is_deleted(tag));
...
static fptu_error fptu_insert_64(fptu_rw *pt, fptu_tag_t tag,
                                 uint_fast64_t v) {
  assert(tag_match_fixedsize(tag, 2));
  assert(!fptu_tag_is_deleted(tag));
...
fptu_error fptu_insert_datetime(fptu_rw *pt, unsigned col,
                                fptu_datetime_t value) {
  if (unlikely(col > fptu_max_cols))
    return FPTU_EINVAL;
