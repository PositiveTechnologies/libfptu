...
__hot std::size_t fptu_field_units(const fptu_field *pf) {
...
__hot const fptu_field *fptu_lookup_ro(fptu_ro ro, unsigned column,
  ...
  if (is_filter(type_or_filter)) {
    ...
    fptu_tag_t tag = fptu_make_tag(column, (fptu_type)type_or_filter);
    for (const fptu_field *pf = begin; pf < end; ++pf) {
      if (pf->tag == tag)
        return pf;
    }
...
__hot fptu_field *fptu_lookup_tag(fptu_rw *pt, fptu_tag_t tag) {
