...
size_t fptu_space(size_t items, std::size_t data_bytes) {
...
fptu_rw *fptu_init(void *space, std::size_t buffer_bytes, std::size_t items_limit) {
...
fptu_rw *fptu_fetch(fptu_ro ro, void *space, std::size_t buffer_bytes,
                    unsigned more_items) {
  if (ro.total_bytes == 0)
  ...
  std::size_t reserve_items = items + more_items;
...
static std::size_t more_buffer_size(const fptu_ro &ro, unsigned more_items,
                               unsigned more_payload) {
  std::size_t items = (size_t)ro.units[0].varlen.tuple_items & fptu_lt_mask;
  std::size_t payload_bytes = ro.total_bytes - units2bytes(items + 1);
...
fptu_rw *fptu_alloc(size_t items_limit, std::size_t data_bytes) {
  if (unlikely(items_limit > fptu_max_fields ||
               data_bytes > fptu_max_tuple_bytes))
    return nullptr;

  std::size_t size = fptu_space(items_limit, data_bytes);
...
fptu_rw *fptu_rw::create(size_t items_limit, std::size_t data_bytes) {
  if (unlikely(items_limit > fptu_max_fields ||
               data_bytes > fptu_max_tuple_bytes))
