...
static __hot const char *fptu_field_check(const fptu_field *pf,
  ...
  static_assert(FPTU_EINVAL == ERROR_INVALID_PARAMETER, "error codemismatch");
  static_assert(FPTU_ENOSPACE == ERROR_ALLOTTED_SPACE_EXCEEDED, "error code mismatch");
...
const char *fptu_check_ro(fptu_ro ro) {
  ...
  std::size_t items = (size_t)ro.units[0].varlen.tuple_items & fptu_lt_mask;
...
const char *fptu_check_rw(const fptu_rw *self) {
  if (unlikely(self == nullptr))
    return "tuple.is_nullptr";

  if (unlikely(self->head < 1))
    return "tuple.head < 1";

  if (unlikely(self->head > self->pivot))
    return "tuple.head > tuple.pivot";

  if (unlikely(self->pivot > self->tail))
    return "tuple.pivot > tuple.tail";

  if (unlikely(self->tail > self->end))
    return "tuple.tail > tuple.end";

  if (unlikely(self->pivot - self->head > fptu_max_fields))
  ...
  if (unlikely(self->junk > self->tail - self->head))
    return "tuple.junk > tuple.size";

  const fptu_field *begin = &self->units[self->head].field;
  const char *pivot = (const char *)&self->units[self->pivot];
  const char *detent = (const char *)&self->units[self->tail];
  std::size_t payload_total_bytes = 0;
  std::size_t payload_junk_units = 0;
  std::size_t junk_items = 0;
  const char *prev_payload = pivot;
  for (const fptu_field *pf = (const fptu_field *)pivot; --pf >= begin;) {
    std::size_t payload_units;
    const char *bug =
  ...
  if (unlikely(self->junk != payload_junk_units + junk_items))
