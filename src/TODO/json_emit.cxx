...
fptu_error emitter::push(size_t length, const char *text) {
  ...
  if (likely(length < sizeof(buffer))) {
    if (likely(length > 0)) {
      const std::size_t space = sizeof(buffer) - fill;
      const std::size_t chunk = (length > space) ? space : length;
      std::memcpy(buffer + fill, text, chunk);
      fill += (unsigned)chunk;
      if (fill == sizeof(buffer)) {
        flush();
        fill = (unsigned)(length - chunk);
        assert(fill < sizeof(buffer));
        std::memcpy(buffer, text + chunk, fill);
      } else {
        assert(chunk == length);
      }
    }
...
struct json : public emitter {
  ...
  void value_dateime(const fptu_datetime_t &value);
  void value_hexadecimal(const uint8_t *data, std::size_t length);
...
void json::value_fp32(const fptu_payload *payload) {
  ...
  if (likely((payload->u32 & exponent_mask) != exponent_mask)) {
    ...
    number(payload->fp32);
  } else if (is_json5() && payload->u32 != FPTU_DENIL_FP32_BIN) {
    assert(__FINITE_MATH_ONLY__ || std::isnan(payload->fp32) ||
           std::isinf(payload->fp32));
    if (payload->u32 & significand_mask) {
      assert(__FINITE_MATH_ONLY__ || std::fpclassify(payload->fp32) == FP_NAN);
      push("NaN");
    } else {
      assert(__FINITE_MATH_ONLY__ ||
             std::fpclassify(payload->fp32) == FP_INFINITE);
      push((payload->i32 < 0) ? '-' : '+');
      push("Infinity");
    }
  } else {
...
void json::value_fp64(const fptu_payload *payload) {
  ...
  if (likely((payload->u64 & exponent_mask) != exponent_mask)) {
    ...
    number(payload->fp64);
  } else if (is_json5() && payload->u64 != FPTU_DENIL_FP64_BIN) {
    assert(__FINITE_MATH_ONLY__ || std::isnan(payload->fp64) ||
           std::isinf(payload->fp64));
    if (payload->u64 & significand_mask) {
      assert(__FINITE_MATH_ONLY__ || std::fpclassify(payload->fp64) == FP_NAN);
      push("NaN");
    } else {
      assert(__FINITE_MATH_ONLY__ ||
             std::fpclassify(payload->fp64) == FP_INFINITE);
      push((payload->i64 < 0) ? '-' : '+');
      push("Infinity");
    }
  } else {
...
void json::value_dateime(const fptu_datetime_t &value) {
  if (value.fixedpoint) {
...
void json::value_hexadecimal(const uint8_t *data, std::size_t length) {
  push('"');
...
void json::field_value(const fptu_field *field) {
  ...
  default:
    const std::size_t length = payload->array_length();
    ...
    for (size_t i = 0; i < length; ++i) {
      ...
      case fptu_datetime | fptu_farray:
        value_dateime(*(const fptu_datetime_t *)ptr);
        static_assert(sizeof(fptu_datetime_t) == 8, "unexpected type size");
...
static int fptu_emit2FILE(void *emiter_ctx, const char *text, std::size_t length) {
...
static int emit2stream(void *emiter_ctx, const char *text, std::size_t length) {
  assert(strnlen(text, length) == length);
