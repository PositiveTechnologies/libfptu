...
The kind of lightweight linearized tuples of optional and obligatory
fields, which are extremely handy to machining, including
(de)serialization and shared memory cases.

Легковесное линейное представление небольших структур из опциональных и
обязательныз полей, в экстремально удобной для машины форме, в том числе
для (де)сериализации и размещении в разделяемой памяти.
...
_libfptu_ или "Позитивные Кортежи" - это _простой_ формат и библиотека
для представления и обмена небольшими структурами из опциональных и
обязательных полей. Можно сказать, что _libfptu_ предлагает гибкость
JSON, скорость нативных структур языка _C_, возможности boost::optional
и boost::variant.

Целью дизайна _libfptu_ была предельно эффективная машинная обработка
небольших порций данных (порядка 10..500 элементов и 1..100 килобайт)
на современных процессорах. При этом совместное использование (чтение)
кортежей в разделяемой памяти является одним из основных и
целеполагающих сценариев (проект 1Hippeus).
...
**1. Удобство для машины и легковесность.** Объем кода минимален, а
внутреннее устройство просто и прозрачно.

**2. Нужно чуть больше места.** _libfptu_ не сжимает данные, а хранит их
в нативном машинном представлении с выравниванием. Поэтому для каждого
...
**3. Очень быстрый доступ.** В _libfptu_ есть простейший индекс. Поэтому
поиск полей и доступ к данным в _libfptu_ очень быстрый.
...
**5. Формируемые кортежи можно изменять.** Можно удалять и
перезаписывать поля, в том числе с изменением размера.
...
экземпляров полей. Поля внутри кортежей строго типизированы и
идентифицируются легковесными токенами. Справочник схемы обеспечивает
трансляцию имен полей в токены, которые используются для доступа к полям
через API. Предлагается до 2000 различимых полей для каждого из 31 типов
данных.

### Поля

"Позитивные кортежи" состоят из типизированных полей двух видов:
предварительно размещенные (preplaсed) и свободные (loose). Основное
отличие между ними в скорости доступа и расходуемых ресурсах. При этом
доступ к полям внутри кортежа производится единообразно через
легковесные токены, вне зависимости от вида поля.


#### Preplaced Fields

Предварительно размещенные поля аналогичны полям плоских структур языка
_C_ и физически всегда присутствуют, т.е. в кортеже соответствующем
некоторой схеме для preplaced-полей всегда выделяется место. Обращение к
preplaced-полю не требует какого-либо поиска и равнозначно чтению данных
по смещению, которое хранится внутри токена.

Накладные расходы доступа к preplaced-полям могут быть сведены к нулю.
Для этого следует определить в исходном коде определить необходимые
структуры, а для их полей сгенерировать статические токен-классы
посредством макроса FPTU_TOKEN. Тогда обращения к preplaced-полям через
такие токены будет выполняться через шаблонные inline-методы, которые
будут свернуты компилятором в непосредственный доступ к данным.
...
Внутри _libfptu_ добавление loose-поля в кортеж сводится к дозаписи
...
При небольшом количестве loose-полей _libfptu_ использует
последовательное сканирование с акселерацией SSE2/AVX/AVX2/AVX512, но
может автоматически переходить на двоичный поиск.
...
(аналогично `repeated` в Protocol Buffers). Для этого определение поля
должно быть добавлено в схему как коллекция.
...
- `text` = Строки UTF8, максимальный размер 262128 байт, терминирующий
ноль не хранится.

- `varbin` = Произвольные последовательности байт (бинарные строки),
максимальный размер 262128 байт.
